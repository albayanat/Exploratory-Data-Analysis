...
NEI_BL_LA_vehicle<-subset(NEI, SCC %in% SCC_vehicle$SCC &  (fips ==  "24510"| fips =="06037"))

##Compute Total emissions of PM2.5 per year for motor vehicle sources
df_BL_LA_vehicle_pm2.5<-ddply(NEI_BL_LA_vehicle, .(year, fips), function(x) sum(x$Emissions))

##Rearrange the dataframe
colnames(df_BL_LA_vehicle_pm2.5)[3]<-"total_emissions"
df_BL_LA_vehicle_pm2.5<- transform(df_BL_LA_vehicle_pm2.5, city = ifelse(fips == "24510", "Baltimore City", "Los Angeles County"))

## Open png device
png("plot6.png", width=480, height=480)

## Construct the plot
plot6<-ggplot(df_BL_LA_vehicle_pm2.5, aes(factor(year), total_emissions, fill = city))
